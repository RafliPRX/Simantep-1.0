...
const Detail_Role_Form_Lv3 = () => {
    ...
    const [role_lv3_detail, setRoleLv1Detail] = useState([]);
    const [nama_role, setNamaRole] = useState(role_lv3_detail.nama_role_b);
    const [kode_role, setKodeRole] = useState(role_lv3_detail.kode_role_b);
    const getDetailRoleLv = async () => {
      try {
            const response = await axios.get(`https://simantepbareta.cloud/API/Admin_API/detail_role_lv3.php?id=${param.id}`, {
                headers: {}
            });
            setRoleLv1Detail(response.data);
            console.log(response.data);
            setNamaRole(response.data.nama_role_b);
            setKodeRole(response.data.kode_role_b);
        } catch (error) {
            console.error(error);
        }
    }
    ...
    const handlePostRole = async (event) => {
      ...
      try {
        const response = await axios.post(`https://simantepbareta.cloud/API/Admin_API/update_role_lv3.php?id=${param.id}`, payload, {
          headers: {
...
export default Detail_Role_Form_Lv3;