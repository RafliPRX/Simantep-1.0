...
const handleAbsentOut = async (event) => {
        ...
        return new Promise((resolve) => {
            canvas.toBlob(async (blob) => {
                const now = new Date();
                const filename = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}-Keluar-Malam.png`;
                ...
                try {
                    const response = await axios.post(
                        `https://simantepbareta.cloud/API/MAWASDIRI/Absen/absent_out_malam.php?id=${param.id}`, 
                        formData,
                        {
                            headers: {
...
export default Absent_Out_Malam

Absent_Out_Malam.propTypes = {
