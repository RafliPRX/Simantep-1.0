...
const Update_Klien_form = () => {   
  ...
  const { id } = useParams();
  console.log("identitas Klien: " + id);
  ...
  const getDetail = async () => {
    try {
      const response = await axios.get(`https://simantepbareta.cloud/API/E-corner/getDetail_klien.php?id=${id}`,{
        headers: {"Content-Type": "application/json"},
      })
      console.log(response.data);
      setDetail(response.data);
      setIdcorner(response.data.id_corner);
      setNamaKlien(response.data.nama_klien);
      setNamaInit(response.data.nama_init);
      setRoom(response.data.rooms);
      setJam(response.data.time);
      setDay(response.data.days);
      setWeek(response.data.id_minggu);
      setMonth(response.data.id_bulan);
    } catch (error) {
      console.log(error);      
    }
  }
  const [id_corner, setIdcorner] = useState(detail?.id_corner || "-");
  const [nama_klien, setNamaKlien] = useState(detail?.nama_klien || "-");
  const handleChangeNamaKlien = (event) => {
    setNamaKlien(event.target.value);
    console.log(event.target.value);
  }
  const [nama_init, setNamaInit] = useState(detail?.nama_init || "-");
  ...
  const [jam, setJam] = useState(detail?.time|| "-");
  const handleChangeJam = (event) => {
    setJam(event.target.value);
    console.log(event.target.value);    
  }
  const [day, setDay] = useState(detail?.days|| "-");
  const handleChangeDay = (event) => {
    setDay(event.target.value);
    console.log(event.target.value);    
  }
  const [week, setWeek] = useState(detail?.id_minggu|| "-");
  const handleChangeWeek = (event) => {
    setWeek(event.target.value);
    console.log(event.target.value);    
  }
  const [month, setMonth] = useState(detail?.id_bulan || "-");
  ...
  const handlePostClientUpdate = async (event) => {
    setIsLoading(true);
    ...
    try {
      const response = await axios.post(`https://simantepbareta.cloud/API/E-corner/update_klien.php?id=${id}`, payload, {
        headers: {
...
export default Update_Klien_form;